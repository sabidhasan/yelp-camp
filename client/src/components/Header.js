  ...
  render() {
    return (
      <nav className={`${this.context.stickyClass} Header`}>
        ...
        <a className='btn bold btn--flat' href='/discover'>
          <i className="fas fa-map"></i>Discover
        </a>
        {this.context.user && !this.context.user.loading ?
          <a href='#' className='btn bold btn--flat' onClick={this.context.signOut}>
            <img className='Header__user-image' src={this.context.user.providerData[0].photoURL} />
            Log Out
          </a>
        : <a href='#' className='Header__login btn bold btn--flat' onClick={this.props.toggleLoginForm}>Sign In</a>
        }
        <a href='#' onClick={this.props.toggleCart} className='Header__cart-icon bold btn btn--flat'>
